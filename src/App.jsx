...
export default function App() {
  ...
  const assetTotals = useMemo(() => {
    return filteredAssetClasses.reduce(
      (acc, row) => {
        acc.investment += row.investment_sgd || 0
        acc.value += row.value_sgd || 0
        acc.profit += row.profit_sgd || 0
        return acc
      },
  ...
  const totalProfitPct =
    assetTotals.investment > 0 ? (assetTotals.profit / assetTotals.investment) * 100 : 0
  ...
  const selectedRows =
    activeTab === "holdings" && selected && selected !== "Cash"
                ...
                <table className="w-full min-w-[900px]">
                  ...
                  <tbody>
                      ...
                      >
                        ...
                        <td
                          className={`py-4 text-right font-semibold ${
                            row.profit_sgd >= 0 ? "text-success" : "text-danger"
                          }`}
                        >
                        ...
                        <td
                          className={`py-4 text-right font-semibold ${
                            row.profit_pct >= 0 ? "text-success" : "text-danger"
                          }`}
                        >
                    ...
                    <tr className="border-t border-border bg-dark/60 font-semibold">
                      ...
                      <td className="py-4 text-right">{assetTotals.investment.toLocaleString()}</td>
                      <td className="py-4 text-right">{assetTotals.value.toLocaleString()}</td>
                      <td className={`py-4 text-right ${assetTotals.profit >= 0 ? "text-success" : "text-danger"}`}>
                        {assetTotals.profit >= 0 ? "▲" : "▼"} {assetTotals.profit.toLocaleString()}
                      </td>
                      <td className={`py-4 text-right ${totalProfitPct >= 0 ? "text-success" : "text-danger"}`}>
                        {totalProfitPct >= 0 ? "▲" : "▼"} {totalProfitPct.toFixed(2)}%
                      </td>
                      <td className="py-4 text-right">{totalPortfolioPct.toFixed(2)}%</td>
                    </tr>
                  </tbody>
                </table>
              </div>
          ...
          >
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between mb-5">
              <div>
                ...
                <p className="text-sm text-gray-500 mt-1">
                  Click an asset class in Overview to load its holdings.
                </p>
              </div>
            ...
            {!selected && (
              <div className="text-gray-400">
                Choose an asset class in the Overview tab to view its holdings.
              </div>
            )}

            {selected && selected !== "Cash" && holdingsLoading && (
                    ...
                    <tbody>
                      ...
                      <tr className="border-t border-border bg-dark/60 font-semibold">
                        ...
                        <td className="py-4 text-right">
                          {selectedRows.reduce((a, b) => a + (b.qty || 0), 0).toLocaleString()}
                        </td>
                        ...
                        <td
                          className={`py-4 text-right ${
                            selectedRows.reduce((a, b) => a + (b.unrealised_gain || 0), 0) >= 0
                              ? "text-success"
                              : "text-danger"
                          }`}
                        >
                          {selectedRows.reduce((a, b) => a + (b.unrealised_gain || 0), 0) >= 0 ? "▲" : "▼"}{" "}
                          {selectedRows.reduce((a, b) => a + (b.unrealised_gain || 0), 0).toLocaleString()}
                        </td>
                        <td className="py-4 text-right">
                          {selectedRows.length > 0
                            ? (
                                (selectedRows.reduce((a, b) => a + (b.unrealised_gain || 0), 0) /
                                  Math.max(selectedRows.reduce((a, b) => a + (b.investment_value || 0), 0), 1)) *
                                100
                              ).toFixed(2)
                            : "0.00"}%
                        </td>
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <td className={`py-2 text-right font-semibold ${selectedRows.reduce((a, b) => a + (b.profit_sgd || 0), 0) >= 0 ? "text-success" : "text-danger"}`}>
                          {selectedRows.reduce((a, b) => a + (b.profit_sgd || 0), 0) >= 0 ? "▲" : "▼"}{" "}
                          SGD {selectedRows.reduce((a, b) => a + (b.profit_sgd || 0), 0).toLocaleString()}
                        </td>
                        <td className="py-2 text-right">
                          {selectedRows.reduce((a, b) => a + (b.portfolio_pct || 0), 0).toFixed(2)}%
                        </td>
            ...
            {selected === "Cash" && holdingsLoading && (
              <div className="text-gray-400">Loading cash groups...</div>
            )}

            {selected === "Cash" && !holdingsLoading && (
              <div className="space-y-6">
                {(holdings || []).map((group, idx) => (
                  <div key={idx} className="bg-dark/60 border border-border rounded-2xl p-5">
                    <div className="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between mb-4">
                      ...
                      <div className="text-sm text-gray-400">
                        Subtotal SGD: {group.subtotal?.value_sgd?.toLocaleString?.() || "0"}
                      </div>
                    </div>
                      ...
                      <table className="w-full min-w-[1000px]">
                        <thead>
                          <tr className="border-b border-border text-sm text-gray-400">
                            ...
                            <th className="py-3 text-right font-semibold">Invested</th>
                            <th className="py-3 text-right font-semibold">Gain</th>
                            <th className="py-3 text-right font-semibold">Gain %</th>
                          </tr>
                        </thead>
                        <tbody>
                          ...
                          <tr className="border-t border-border bg-hover/40 font-semibold">
                            ...
                            <td className="py-4 text-right">{group.subtotal?.market_value?.toLocaleString?.() || "0"}</td>
                            <td className="py-4 text-right">{group.subtotal?.investment_value?.toLocaleString?.() || "0"}</td>
                            <td className="py-4 text-right">
                              {group.subtotal?.profit_sgd >= 0 ? "▲" : "▼"} {group.subtotal?.profit_sgd?.toLocaleString?.() || "0"}
                            </td>
                            <td className="py-4 text-right">
                              {group.subtotal?.investment_value > 0
                                ? ((group.subtotal.profit_sgd / group.subtotal.investment_value) * 100).toFixed(2)
                                : "0.00"}%
                            </td>
                          </tr>
                        </tbody>
                ...
                <div className="bg-dark rounded-2xl p-5 border border-border/70">
                  <div className="text-lg font-semibold mb-3">Cash Grand Total</div>
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-3 text-sm">
                    <div className="bg-card rounded-xl p-4 border border-border">
                      <div className="text-gray-400 mb-1">Market Value</div>
                      <div className="font-semibold">
                        SGD {(portfolio.cash_groups?.grand_total?.value_sgd || 0).toLocaleString()}
                      </div>
                    </div>
                    <div className="bg-card rounded-xl p-4 border border-border">
                      <div className="text-gray-400 mb-1">Investment Value</div>
                      <div className="font-semibold">
                        SGD {(portfolio.cash_groups?.grand_total?.investment_value || 0).toLocaleString()}
                      </div>
                    </div>
                    <div className="bg-card rounded-xl p-4 border border-border">
                      <div className="text-gray-400 mb-1">Profit</div>
                      <div className="font-semibold">
                        SGD {(portfolio.cash_groups?.grand_total?.profit_sgd || 0).toLocaleString()}
                      </div>
                    </div>
                    <div className="bg-card rounded-xl p-4 border border-border">
                      <div className="text-gray-400 mb-1">Rows</div>
                      <div className="font-semibold">{(holdings || []).reduce((a, b) => a + (b.rows?.length || 0), 0)}</div>
                    </div>
                  </div>
                </div>
              </div>
