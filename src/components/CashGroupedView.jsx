const CashGroupedView = ({ groups, selectedType, setSelectedType }) => {
  return (
            ...
            <table className="w-full table-fixed">
              <thead>
                <tr className="border-b border-border text-sm text-gray-400">
                  <th className="py-3 pl-2 pr-3 text-left font-semibold w-[34%]">
                    Name
                  </th>
                  <th className="py-3 px-3 text-right font-semibold w-[22%]">
                    Market Value (SGD)
                  </th>
                  <th className="py-3 px-3 text-right font-semibold w-[22%]">
                    Investment (SGD)
                  </th>
                  <th className="py-3 px-3 text-right font-semibold w-[11%]">
                    Gain
                  </th>
                  <th className="py-3 px-3 text-right font-semibold w-[11%]">
                    Gain %
              ...
              <tbody>
                {group.rows?.map((h, i) => (
                  <tr key={i} className="border-b border-border/60 hover:bg-hover/40 transition-colors">
                    <td className="py-4 pl-2 pr-3 font-semibold truncate">
                      {h.asset || "-"}
                    </td>
                    ...
                    <td
                      className={`py-4 px-3 text-right font-semibold ${
                        Number(h.unrealised_gain_pct || 0) >= 0
                          ? "text-success"
                          : "text-danger"
                      }`}
                    >
                ...
                <tr className="border-t border-border bg-hover/40 font-semibold">
                  ...
                  <td
                    className={`py-4 px-3 text-right ${
                      Number(group.subtotal?.profit_sgd || 0) >= 0
                        ? "text-success"
                        : "text-danger"
                    }`}
                  >
                  ...
                  <td className="py-4 px-3 text-right">
                    {group.subtotal?.investment_value > 0
                      ? pct2(
                          (group.subtotal.profit_sgd /
                            group.subtotal.investment_value) *
                            100
                        )
                      : "0.00%"}
                  </td>
